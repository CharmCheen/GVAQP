...
- **ECP (Event-Coverage Policy) redesign (T010/T025/T026/T027).** T010 is the ECP mainline (`outputs/ecp_event_coverage_policy_v1/ECP_DESIGN.md`); T025/T026/T027 are COMPLETE (Round 21). Findings: (1) formation gap is temporal granularity (sub-1s events in 10s bins) + proxy-zero discovery, NOT gap-bridging (positives already contiguous, max_internal_gap=0 for all 74 events); (2) BRIDGE is the most efficient arm (0.474 vs DISCOVER 0.277 positive rate) but under-used by hand weights; (3) hand-designed ECP bandit runs strict-replay and matches/beats HTS-EC-safe on proxy-informative realcartest (e.g. 0.300 vs 0.183 @0.30 on realcartest_0_1570) but is still weak on proxy-zero dataset3 (≈0 at low budget, below B7-strict at 0.30). The claim "ECP improves low-budget event coverage over B7-core" remains **not supported** until T028 (reweight + learned policy) lands. ECP does not alter the default selector or Core/Halo release.
