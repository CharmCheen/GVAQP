...
- **ECP (Event-Coverage Policy) redesign (T010/T025/T026/T027).** T010 is now the ECP mainline (`outputs/ecp_event_coverage_policy_v1/ECP_DESIGN.md`); T025 (AnchorBridge shadow) is active. ECP reframes the objective as an event-level contextual bandit (arms = discover/bridge/certify/zero-proxy/stop; reward = marginal event-recall/precision/IoU) above the discovery executor (HTS-EC / EventLift-DC). Until T025 shows a measurable formation gap and T027 shows a hand-designed bandit beats the fixed pipeline under strict replay, the claim "ECP improves low-budget event coverage over B7-core / EventLift-DC" is **not supported**. ECP does not alter the default selector or Core/Halo release.
