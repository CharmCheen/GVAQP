...
### ECP-utility-c1 generalizes on proxy-informative segments (LOSO validated)

- **Allowed wording**: "Under leave-one-segment-out validation (6-fold), the ECP event-utility ranking policy (c1) generalizes on proxy-informative realcartest: 0 regressions on held-out segments, 4/9 budget cells c1 > v2 (up to +0.143), 3/9 ties, 2/9 ceiling-reached. c1 hits the oracle ceiling on realcartest_0_1570 @0.20/0.30 and realcartest_3200_3830 @0.30. The policy learns to prefer DISCOVER+CERTIFY over BRIDGE, and this transfers to held-out segments."
- **Source**: `outputs/ecp_event_coverage_policy_v1/t028c2_loso_report.md`, `t028c2_loso_synthesis.md`.
- **Track**: strict_replay (c1 and v2); OFFLINE_CEILING (oracle ceiling for reference only).
- **Caveats**: c1 does NOT generalize on proxy-zero dataset3 (candidate-generator bottleneck; ceiling also ~0 there). The improvement is over v2 hand-designed weights, not over B7-core. Per-segment, per-budget reporting required; do not aggregate into a single headline number.
...
- **ECP (Event-Coverage Policy) redesign (T010/T025..T028e).** T010 is the ECP mainline (`outputs/ecp_event_coverage_policy_v1/ECP_DESIGN.md`); T025-T028e are COMPLETE (Rounds 21-25). T028c-0/T028c-1/T028c-2 established the event-utility selector (c1) that generalizes on proxy-informative realcartest (LOSO: 0 regressions, 4/9 improvements, ceiling-reached on 2 cells). T028e added proxy-free candidate arms (VDC, MIDBAND, LARGEST_GAP, LOCAL_GAP_FLANK, SPACE_FILLING). T028e-0 ceiling audit: dataset3_1200_2400 ceiling lifts +0.083. T028e-1 strict-replay LOSO: c2 matches c1 on realcartest AND beats v2 on 2/9 dataset3 cells (most importantly dataset3_0_1200 @0.20/0.30 c2=0.167 vs v2=0.000, ceiling was 0). Claim status: **ECP-c2 generalizes on proxy-informative realcartest and adds value on proxy-zero dataset3_0_1200 (LOSO-validated)**. T028d (IPS/DR) deferred until stochastic logger. ECP does not alter default selector or Core/Halo release.
