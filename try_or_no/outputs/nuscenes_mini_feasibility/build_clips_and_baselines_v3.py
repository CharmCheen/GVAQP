"""
Build ground-truth clips and run baseline smoke test (v3).
Fix: treat instances with 0 GT clips + 0 predicted clips as perfect score.
"""
...
def evaluate_instance(predicted, labels, gt_clips, tau):
    ...
    # FIX: if both GT and predicted have 0 clips, this is a perfect prediction
    if len(gt_intervals) == 0 and len(pred_clips) == 0:
        return {
            "clip_recall": 1.0,
            "clip_precision": 1.0,
            "clip_f1": 1.0,
            "mean_iou": 1.0,
            "boundary_error_samples": 0.0,
        ...
        for tau in tau_values:
            for seed in seeds:
                ...
                for method_name, instance_metrics in method_results.items():
                    ...
                    elif method_name == "fixed_rate_k10":
                        oracle_calls = sum(1 for it in sequences for i in range(len(sequences[it])) if i % 10 == 0)
