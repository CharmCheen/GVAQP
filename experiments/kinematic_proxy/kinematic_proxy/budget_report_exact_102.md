...
- strict label available: True (exact labels include vlm_affected_ego and vlm_risk_level)
- high-confidence label available: False (not used for exact 102-label evaluation)
...
## Exact 102-Clip Answers

1. Positive rates: broad=0.941 (96/102), ego_relevant=0.804 (82/102), strict=0.667 (68/102).
2. Strict label reduces the broad positive rate, so it is usable for a less saturated pseudo-GT view.
3. broad: positive events=2, average positive run length=48.000 clips.
3. ego_relevant: positive events=4, average positive run length=7.455 clips.
3. strict: positive events=7, average positive run length=8.500 clips.
4-5. broad: low-budget best clip recall is `ensemble_all_proxy` (0.141); best event recall is `ensemble_all_proxy` (1.000).
4-5. ego_relevant: low-budget best clip recall is `ensemble_count_naive` (0.165); best event recall is `temporal_nms_naive` (0.938).
4-5. strict: low-budget best clip recall is `top_count` (0.199); best event recall is `top_kinematic` (0.893).
6. Clip-level and event-level conclusions still diverge: ego_relevant: clip `ensemble_count_naive` vs event `temporal_nms_naive`; strict: clip `top_count` vs event `top_kinematic`.
7-11. broad: uniform_expansion avg clip recall=0.226 vs top_count=0.236; temporal-NMS best avg event recall=1.000 vs top/ensemble proxy best=1.000; proxy-anchor sweep avg event recall=0.843 vs uniform-anchor=0.500; top_count/top_naive avg clip recall=0.236/0.236; top_kinematic avg clip recall=0.236.
7-11. ego_relevant: uniform_expansion avg clip recall=0.264 vs top_count=0.274; temporal-NMS best avg event recall=0.958 vs top/ensemble proxy best=0.833; proxy-anchor sweep avg event recall=0.509 vs uniform-anchor=0.250; top_count/top_naive avg clip recall=0.274/0.244; top_kinematic avg clip recall=0.211.
7-11. strict: uniform_expansion avg clip recall=0.309 vs top_count=0.324; temporal-NMS best avg event recall=0.857 vs top/ensemble proxy best=0.571; proxy-anchor sweep avg event recall=0.371 vs uniform-anchor=0.190; top_count/top_naive avg clip recall=0.324/0.275; top_kinematic avg clip recall=0.213.
12. Current exact-label evidence supports temporal-aware budget allocation as an allocation problem with separate dense clip recovery and event coverage objectives.
13. Kinematic proxy is not uniformly weak in this exact-label run, but it should only be developed if the gain is stable across variants and budgets.
