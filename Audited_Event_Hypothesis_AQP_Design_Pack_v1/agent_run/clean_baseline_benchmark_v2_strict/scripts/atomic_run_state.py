...
def main() -> None:
    ...
    parser.add_argument("--physical-vlm-calls-unknown", action="store_true")
    parser.add_argument("--physical-attempts-started", type=int)
    parser.add_argument("--uncertain-attempts", type=int)
    ...
    for key in [
        ...
        "physical_attempts_started", "uncertain_attempts",
        "completed_baseline_runs", "failed_baseline_runs",
    ]:
        value = getattr(args, key)
        if value is not None:
            state[key] = value
    if args.physical_vlm_calls_unknown:
        state["physical_vlm_calls"] = None
